...
export default function Staff() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      {lead && (
        <Link to={`/staff/${slugify(lead.name)}`} className="mt-6 block">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card ring-1 ring-amber-100 lg:flex lg:items-center lg:gap-6">
            <div className="mx-auto h-40 w-40 md:h-44 md:w-44 lg:h-48 lg:w-48 overflow-hidden rounded-2xl border border-slate-200 bg-white">
              {/* móvil: object-contain para no cortar caras; md+: cover */}
              <img
                src={lead.img}
                alt={lead.name}
      ...
      {/* Asistentes: todos del mismo tamaño; una sola fila en desktop (5 col) */}
